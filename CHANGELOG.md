...
Kết quả kiểm thử: `tests/unit/test_stt_eval_failure_decomposition.py` — **56 passed**. Toàn bộ `tests/unit/` (số liệu terminal thật của pytest, không phải thuộc tính `tests` trong file JUnit XML — xem đính chính bên dưới) — baseline tại commit gốc `2b73a49` (= `main`): **1008 collected, 1008 passed, 0 failed, 0 skipped** (khớp chính xác với GitHub Actions CI run #126: 1008 collected; phần passed/skipped của CI — 1005 passed/3 skipped — là khác biệt local-vs-CI đã biết từ trước, không liên quan nhánh này). Trạng thái cuối cùng đã push của nhánh này (commit `42098d6`): **1064 collected, 1064 passed, 0 failed, 0 skipped** (cộng thêm 50 subtests passed, không đổi so với baseline). Đúng bằng **+56**, khớp chính xác số test mới.

**Ghi chú về chênh lệch +46/+56**: commit `42098d6` đã push và bản nháp đầu của mục này từng ghi "46 test mới"/"1054 collected" — con số đó đúng với nội dung file test tại một thời điểm giữa phiên làm việc, trước khi 10 test nữa (`TestComputeTextSimilarityStats` và 2 case bổ sung của `TestRenderMarkdownReport`) được thêm vào TRONG CÙNG commit đó để khóa cứng metric tương đồng văn bản phụ trợ — tất cả trước khi commit được push. Nội dung commit message công khai không bị viết lại (theo đúng yêu cầu không amend commit đã push); chỉ tài liệu ở commit follow-up này được sửa để phản ánh đúng bằng chứng cuối cùng. File thực tế trên nhánh, kể từ `42098d6`, luôn có đủ 56 test — 56/1064 là con số đúng duy nhất, không dùng lại 46/1054 nữa.

**Đính chính (phát hiện qua việc người dùng tự kiểm tra độc lập GitHub Actions CI #126 trên đúng commit gốc, 2026-09-02): một bản nháp trước đó của báo cáo này đã nhầm số `tests` trong file JUnit XML với số "collected" thật của pytest — hai con số này KHÔNG giống nhau trong repo này.** `pytest-subtests` (dependency đã khai báo trong `pyproject.toml`) tạo thêm một `<testcase>` riêng trong JUnit XML cho mỗi lần gọi `subtests.test()`, ngoài các `<testcase>` của những test thường chứa nó — nhưng KHÔNG cộng vào số "collected N items" hay dòng "N passed" thật của pytest ở terminal. Tại baseline (`2b73a49`), pytest báo "1008 passed, 50 subtests passed" (collected = 1008), trong khi JUnit XML báo `tests=1058` = 1008 + 50 — bị thổi phồng đúng bằng 50 subtests có sẵn từ trước, không liên quan đến nhánh này. Ở trạng thái cuối cùng của nhánh này, pytest báo "1064 passed, 50 subtests passed" (collected = 1064), JUnit sẽ báo `tests=1114` = 1064 + 50 — cũng bị thổi phồng đúng bằng 50 subtests đó. Số liệu đúng, xác thực là số ở đoạn trên, lấy trực tiếp từ dòng tóm tắt terminal thật của pytest (qua `subprocess.run()`, không dựa vào file JUnit XML, và không qua pipe shell `tail`/`tr`, và không truyền thêm `-q` thứ hai trên dòng lệnh — `pyproject.toml` đã có sẵn `-q` trong `addopts`, truyền thêm lần nữa sẽ làm pytest im lặng tới mức ẩn luôn dòng tóm tắt cuối). Không có bằng chứng lịch sử (CI run #126, file JUnit gốc) nào bị sửa — chỉ sửa cách diễn giải trong tài liệu này.
