...
> **Current baseline note (updated 2026-09-02, after `eval/stt-real-mic-baseline-correction` merged `origin/main`):** `main`'s `CHANGELOG.md` development history now reaches **v4.4.0** — unlike every prior milestone since v4.0.1 (v4.1.x–v4.3.2, all dev-history labels that deliberately did NOT move `jarvis.__version__`), **v4.4.0 is the first milestone where the CHANGELOG heading and the runtime version literal moved together**: `jarvis.__version__` was bumped `4.1.0 → 4.4.0` as part of that same commit (`4bebc42`, "fix(prod): v4.4.0 - Fix 3 production bugs + expand Tier-1 rules"). Do not assume future milestones will keep this pattern — check `jarvis/__init__.py` directly rather than inferring the runtime version from a CHANGELOG heading. v4.4.0 fixed three production bugs (a `parse_intent(None)` crash, a `WakeWordDetector` false-positive on pure-tone audio, and 23 unguarded `subprocess.run(text=True)` call sites lacking `encoding=`), expanded Tier-1 rule_engine coverage (stop/settings/screen-off keywords), and adjusted the STT eval's old (now superseded) phrase categorization. It sits on top of the earlier v4.3.2-era maintenance work and v4.3.1-era work (real-microphone STT evaluation, sandbox/security hardening, `PromptGuard`, comms rate limiting, email IMAP hardening, Secrets Manager). **Also merged in the same `origin/main` pull, not yet its own CHANGELOG milestone as of this exact commit** (their CHANGELOG documentation, commit `857d729` "v4.5.0", is on `origin/main` but was not part of this particular merge — do not describe it as present here): SecretsManager wired into 6 more production modules (`jarvis/core/app.py`, `stt/engine.py`, `vision/screen.py`, `web/weather.py`, `agent/graph.py`, `workers/notification_hub.py`), a new N=152 text-routing eval (`tests/eval/routing_eval_n150.py`, Wilson-CI text-only routing accuracy — separate evidence from the acoustic STT eval, see §"STT" below), an emoji-detection regex extended to BMP ranges, and an audio echo-feedback-loop fix (`jarvis/core/app.py`, wake-word-triggered `unknown_intent` no longer speaks "Xin lỗi" to avoid the mic re-hearing its own TTS). CHANGELOG milestone headings are **development-history labels, not formal releases** — the latest actual formal GitHub Release/tag remains `v4.0.1` (unaffected by any of this). `config/default_config.yaml`'s `system.version` (`1.0.0`) remains a separate, currently-unused field (see §1A), unaffected. Sections 4, 7, 9, and 10 below are **historical v4.0.1 release record**, kept for context; do not read them as describing the current `main`. Always trust `docs/PROJECT_STATE.md` and actual Git state for the current baseline.
