...
7. **Report delivery (`_send_morning_report`)**: earlier source documentation described Telegram delivery, but that stale docstring has been corrected (2026-09-01). The current implementation — and its docstring — persist the completed task's Markdown report to a local file only: `%LOCALAPPDATA%\JARVIS\logs\night_report_<task_id>.md` (or `~/.jarvis/logs/...` when `LOCALAPPDATA` is unset). **No Telegram or other comms delivery is implemented today.**
