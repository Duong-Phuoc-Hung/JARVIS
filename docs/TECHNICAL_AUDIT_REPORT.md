...
| 8 | **P0: Passive-trigger runaway / unbounded external-launch authority** (`jarvis/core/app.py::_on_gesture_event`/`_on_wake_word_triggered`, `jarvis/plugins/spotify.py`/`chrome.py`/`cursor.py`, `jarvis/automation/control.py::ComputerController.open_app()`/`open_website()`, `jarvis/stt/engine.py::STTEngine._on_config_reloaded()`) — phát hiện sau sự cố sản xuất thật: JARVIS khiến một máy Windows đạt tải CPU/GPU/RAM cực đoan, liên tục mở nhiều ứng dụng (Settings/Chrome/Spotify/khác) đến mức phải tắt máy bằng nút nguồn vật lý; một người dùng độc lập thứ hai tái hiện hành vi tương tự | ✅ **RESOLVED** — nhánh `fix/voice-control-truthfulness`, **CHƯA MERGE tại thời điểm viết** | **Không có log sự cố thật khả dụng để xác nhận** — mọi phát hiện đến từ kiểm toán mã nguồn trực tiếp, không phải đọc log. Xác nhận độc lập: (a) `gesture.patterns.double_clap.actions` mặc định trao quyền không cần xác thực cho một trigger âm học thụ động khởi chạy 5 side-effect hạng nặng (spotify/2×chrome/tts/cursor); (b) không có launch nào (Spotify/Chrome/Cursor/`open_app`/`open_website` — bao gồm cả `"settings"` → `ms-settings:`, khớp trực tiếp triệu chứng "Settings" trong báo cáo) có rate-limit; (c) cooldown gesture cũ chỉ có khoảng-cách-tối-thiểu, không giới hạn tổng số trigger theo thời gian; (d) `STTEngine._on_config_reloaded()` tái tạo vô điều kiện một `FasterWhisperSTT` nặng (kèm preload mặc định cũ) trên MỌI lần hot-reload cấu hình, kể cả không liên quan STT. Sửa: module mới `jarvis/core/runaway_guard.py` (`PassiveTriggerGuard` — cửa sổ trượt max-trigger + lockout, `LaunchDedupeGuard` — dedupe/rate-limit khởi chạy) nối vào tất cả điểm gọi đã xác nhận; `gesture.patterns.double_clap.allow_side_effect_fanout` (mặc định `false`) biến fanout thành opt-in; `STTEngine._on_config_reloaded()` so sánh snapshot cấu hình liên quan đến engine trước khi tái tạo; `FasterWhisperSTT` mặc định lazy-load (`preload=False`). Không đổi `SafetyGateInterceptor`/RBAC. Kiểm chứng: 48 test mới (`test_runaway_guard.py` 21 + `test_runaway_hardening.py` 27), `tests/unit/` toàn bộ: 1633 collected, 1632 passed, 1 skipped, 0 failed — không có side-effect thật nào (app/audio/GPU/tiến trình) xảy ra trong toàn bộ kiểm chứng. Xem `docs/SECURITY_ARCHITECTURE.md` §7, `CLAUDE.md`'s "runtime-runaway / resource-exhaustion hardening invariant", và `docs/PROJECT_STATE.md`'s checkpoint `0-PRE5`. |
...
chưa sửa) và hạng mục #8 (đã sửa, chưa merge) ở trên.
