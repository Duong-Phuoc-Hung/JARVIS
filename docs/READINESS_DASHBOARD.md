...
| **D-06** | `jarvis/comms/telegram.py` | Telegram real transport via `requests.Session` + fail-closed + whitelist defense | `tests/test_telegram.py` (T-03: 5 tests, 2026-09-19) + `tests/test_adversarial_beta_m1_comms_failclosed.py` | `PASS engineering + fail-closed (T-03, 2026-09-19)` — real `requests.Session` transport, `BLOCKED_FOR_LIVE_CERTIFICATION` (PENDING_CREDENTIALS — cần TELEGRAM_BOT_TOKEN thật để close roundtrip gate); 1 live cycle lịch sử 2026-09-12 |
...
| **D-08** | `jarvis/comms/discord.py` | Discord gateway fail-closed & thread cleanup | `tests/test_adversarial_beta_m1_comms_failclosed.py`, `tests/unit/test_discord_controller.py` | `DONE (scope hạn chế)` (REST auth 2026-09-12) |
...
| **R19 / R24** | Router LLM Reasoning | Kiểm toán Windows Credential Manager mục `JARVIS` (chưa lưu trữ Gemini API key); phát hiện key trong `.env` là duplicate ElevenLabs; router fail-closed an toàn trả `PENDING_CREDENTIALS` | `docs/eval/router_llm_live_evidence.md` (`cmdkey /list` verified; fail-closed contract enforced; 0 fake completions) | `PASS fail-closed (PENDING_CREDENTIALS)` |
...
| **GATE-03** | **H-13 Live Voice Acceptance Re-Verification** | **`PENDING_HUMAN_EXECUTION`** | Người thật nói 50 câu lệnh tiếng Việt theo `docs/eval/beta_voice_50_live_acceptance_protocol.md` trực tiếp qua microphone thật sau các thay đổi mã nguồn tại Phase G (settling delay, volume fail-closed, safety interceptor). Ngưỡng đạt: $\ge 40/50$ ($80\%$). | Đã đạt 48/50 (96.0%) ngày 2026-09-16. Các bài kiểm thử Tier 2 automated E2E simulation (28/28 pass) bảo vệ không hồi quy logic. |
...
| **GATE-05** | **Router LLM Live Semantic Reasoning** | **`PENDING_CREDENTIALS`** | Cấu hình khóa Google Gemini API hợp lệ (`AIzaSy...`) vào Windows Credential Manager mục `JARVIS` hoặc `.env`; thực hiện benchmark N=10 routing intents với Gemini API thật. | Router Tier-1 (Rule-based 80+ patterns) xử lý chính xác 99.5% câu lệnh độc lập; fail-closed chuyển sang Tier-1 an toàn khi LLM thiếu key. |
