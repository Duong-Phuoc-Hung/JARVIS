...
| **R20 — TieredSTT Multi-Domain WER** | Đo lường thực nghiệm WER trên 3 operational voice domains đóng | **`PASS runtime`** | `docs/eval/tiered_stt_wer_domain.md` | FasterWhisper `large-v3` trên CUDA: Command WER **8.37%** (8.50%), Free-form Vietnamese WER **3.72%**, Combined WER **5.84%** (5.88%) trên 60 audio samples; vượt trội so với baseline. |
