...
| **R19 / R24 — Router LLM Live Reasoning** | Kiểm thử semantic routing N=10 qua Gemini API với key thật | **`PASS runtime (9/10 = 90%, 2026-09-20)`** | `docs/eval/router_llm_live_evidence_v2.md` + `docs/eval/router_llm_live_evidence_v2.json` | `GEMINI_API_KEY=AIzaSy...` (39 chars) sync từ `GOOGLE_API_KEY` vào `.env`; benchmark N=10 Vietnamese intents với `gemini-flash-lite-latest` API thật; 9/10 PASS, avg 979ms; intent #2 "tìm kiếm thời tiết" → LLM chọn `weather_query` (semantically correct); free tier 5 RPM, 15s delay giữa requests. |
