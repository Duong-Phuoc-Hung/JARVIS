...
<sub>**Phiên bản mã nguồn / phát triển (source/runtime, `jarvis.__version__`): 5.0.1** trên `main` — nâng cấp toàn diện đường ống âm thanh giọng nói, bảo mật và lưu trữ. Đợt kiểm toán toàn diện 7 phân hệ và khắc phục chuẩn Seam-First TDD (Phase 7 & Phase 8, 2026-09-07) đã giải quyết triệt để 8 khuyết tật trọng yếu Bảng Đỏ (D1–D8): thiết lập hợp đồng Fail-Closed tuyệt đối cho Zalo OA, Discord Polling, Telegram Command Dispatcher, CDP Browser Driver, Windows Audio Master Volume, Network Scanner PacketCapture và Audio Engine Device Probe. Phase 9 (2026-09-10) bổ sung: IMAPEmailReader với `imaplib` thật (fail-closed NOT_CONFIGURED khi thiếu credentials), vá TTS SAPI5 Priority 4 giả mạo thành công (return False), 20 IMAP unit tests + 4 volume fail-closed tests mới (tổng suite 100% green, 0 failures).</sub>
