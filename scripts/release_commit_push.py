...
def main():
    ...
    res1 = subprocess.run(["git", "add", "-A"], capture_output=True, text=True)
    print("STDOUT:", res1.stdout)
    print("STDERR:", res1.stderr)
    print("RET:", res1.returncode)
    
    print("\n=== Committing ===")
    res2 = subprocess.run(["git", "commit", "-m", "feat: v4.7.0 - Sprint 2 Acoustic & UX Hardening"], capture_output=True, text=True)
    ...
    print("\n=== Git Status ===")
    res4 = subprocess.run(["git", "status"], capture_output=True, text=True)
    print("STDOUT:\n", res4.stdout)
    print("STDERR:\n", res4.stderr)

    print("\n=== Git Log -1 ===")
    res5 = subprocess.run(["git", "log", "-1"], capture_output=True, text=True)
    print("STDOUT:\n", res5.stdout)
    print("STDERR:\n", res5.stderr)
